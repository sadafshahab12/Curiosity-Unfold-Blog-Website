...
const getData = async () => {
  const fetchData = await client.fetch(groq`*[_type == "category"][0...3]`);
  return fetchData;
};
        ...
        {data.map((category: Category, index: number) => {
          ...
          return (
            <Link href={`/blog/${category.slug.current}`} key={index}>
              <div
                className="shadow-boxShadow relative xs:mt-24 xxs:mt-20 mt-16 bg-cardBg bg-cover bg-center bg-no-repeat  rounded-xl py-5 "
                key={index}
              >
                ...
                <div className="md:mt-[10rem] xs:mt-[14rem] xxs:mt-[10rem] mt-[7rem] xxs:px-8 px-4 xs:space-y-4 space-y-3">
                  ...
                  <p className="xs:text-sm text-[0.8rem]  text-slate-700">
